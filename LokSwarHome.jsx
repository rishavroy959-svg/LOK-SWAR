 ...
 * 2. Central Intake Card: 🏛️ लोक स्वर title, Subtitle, Audio 🔊 button, unified Voice/Text/Camera bar, and Deep Green/Blue submit action.
 * 3. Multilingual Speech Synthesis (TTS): Hindi (hi-IN), Odia (or-IN), English (en-IN).
 * 4. Slide-Out Drawer: All 8 core information clusters & decision dashboards.
 * 5. Clean Footer: Quick voice sample buttons, zero bottom network clutter.
 */

export default function LokSwarHome({ onNavigateTab }) {
  // -------------------------------------------------------------
  // State
  // -------------------------------------------------------------
  const [isDarkMode, setIsDarkMode] = useState(false); // Light Mode (#F4F9F6) vs Soft Blue (#3B4D61)
  const [selectedLang, setSelectedLang] = useState('hi'); // 'hi' | 'or' | 'en' | 'bn'
  ...
  // Multilingual Strings & TTS Scripts
  const i18n = {
    hi: {
      brandTitle: "लोक स्वर",
      ...
      ttsPrompt: "लोक स्वर नागरिक सेवा में आपका स्वागत है। माइक बटन दबाकर अपनी समस्या बोलें, या कैमरा से फोटो लगाएं, फिर नीचे हरा बटन दबाकर रिपोर्ट जमा करें।"
    },
    or: {
      brandTitle: "ଲୋକ ସ୍ୱର",
      brandSub: "OFFICIAL CIVIC INTELLIGENCE • AC-134 SUNDARGARH",
      ...
      ttsPrompt: "ଲୋକ ସ୍ୱର ନାଗରିକ ସେବାକୁ ସ୍ୱାଗତ। ମାଇକ୍ ବଟନ୍ ଦବାଇ ଆପଣଙ୍କ ସମସ୍ୟା କୁହନ୍ତୁ କିମ୍ବା ଫଟୋ ଦେଇ ସବୁଜ ବଟନ୍ ଦବାନ୍ତୁ।"
    },
    en: {
      brandTitle: "Lok Swar",
      brandSub: "OFFICIAL CIVIC INTELLIGENCE • AC-134 SUNDARGARH",
      ...
      ttsPrompt: "Welcome to Lok Swar civic intelligence engine. Tap the microphone button to speak your problem, or tap camera to attach a photo, then press the submit button."
    },
    bn: {
      brandTitle: "লোক স্বর",
      brandSub: "OFFICIAL CIVIC INTELLIGENCE • AC-134 SUNDARGARH",
      ...
      ttsPrompt: "লোক স্বর নাগরিক সেবায় স্বাগতম। মাইক বোতাম টিপে আপনার সমস্যা বলুন বা ছবি তুলে রিপোর্ট জমা দিন।"
    }
  };
  ...
  // -------------------------------------------------------------
  // Multilingual Speech Synthesis (TTS) - Hindi, Odia, English
  // -------------------------------------------------------------
  const handleMultilingualReadAloud = () => {
    ...
    const targetCode = langCodeMap[selectedLang] || 'hi-IN';
    const textToSpeak = t.ttsPrompt;
    ...
    // Fallback handler if specific regional voice isn't installed locally
  ...
  // -------------------------------------------------------------
  // Voice Recording Toggle with Waveform Visualizer
  // -------------------------------------------------------------
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    if (!inputText && !photoPreview) {
      handleMultilingualReadAloud();
      return;
    }
    playChime('success');
  ...
  return (
    <div
      className={`min-h-screen flex flex-col justify-between select-none relative font-sans transition-colors duration-300 ${
        isDarkMode
          ? 'bg-[#3B4D61] text-[#FFFFFF]' // Soft Blue Theme
          : 'bg-[#F4F9F6] text-slate-800'  // Light Mint/Porcelain
      }`}
    >
      {/* -------------------------------------------------------------
          1. TOP HEADER (Empty Center, Left [≡], Right Utilities)
          ------------------------------------------------------------- */}
      <header className="w-full px-4 md:px-8 py-4 flex items-center justify-between z-30">
        
        {/* Left: Primary Hamburger Menu Button [≡] */}
        <button
          onClick={() => {
            playChime('tap');
        ...
        {/* Center: [EMPTY] as per requirement */}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="relative">
            ...
            >
              ...
              {unreadNotifs > 0 && (
                <span className="absolute top-2.5 right-2.5 w-2.5 h-2.5 rounded-full bg-rose-500 ring-2 ring-white animate-pulse" />
              )}
            </button>
          ...
          <div className="relative">
            <select
              ...
              className={`h-[48px] px-3 text-xs font-bold rounded-2xl border outline-none cursor-pointer active:scale-95 transition-all shadow-sm ${
                isDarkMode
                  ? 'bg-[#4A6278] border-[#5E7A94] text-white'
            ...
            >
              ...
              <option value="bn" className={isDarkMode ? 'bg-[#3B4D61]' : ''}>বাংলা (BN)</option>
            </select>
          </div>

          {/* ☀️/🌙 Soft Blue Night Mode Toggle */}
          <button
            onClick={() => {
              playChime('tap');
              setIsDarkMode(!isDarkMode);
            }}
            className={`min-w-[48px] min-h-[48px] flex items-center justify-center rounded-2xl border transition-all duration-200 active:scale-95 shadow-sm ${
              isDarkMode
          ...
          >
            {isDarkMode ? (
              <Sun className="w-5 h-5 stroke-[2.2] text-amber-300" />
            ) : (
      ...
      {/* -------------------------------------------------------------
          2. CENTRAL INTAKE CARD: 🏛️ लोक स्वर + Unified Input
          ------------------------------------------------------------- */}
      <main className="flex-1 max-w-2xl w-full mx-auto px-4 flex flex-col justify-center py-6">
        
        {/* Main Central White/Soft-Blue Container Card */}
        <div
          className={`rounded-3xl border-2 p-6 md:p-8 shadow-2xl transition-all duration-300 relative ${
            isDarkMode
              ? 'bg-[#4A6278] border-[#5E7A94] shadow-black/20'
              : 'bg-white border-[#D8E2DC] shadow-slate-200/80'
          }`}
        >
          {/* Card Top Row: Centered "🏛️ लोक स्वर" Title + Top-Right Audio Readout Button */}
          <div className="relative mb-6 text-center">
            ...
            <div className="flex flex-col items-center">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h2
                  className={`text-2xl md:text-3xl font-black tracking-tight font-serif ${
            ...
            {/* Top-Right Multilingual Audio Readout Button (🔊) inside card */}
            <button
              onClick={handleMultilingualReadAloud}
              className={`absolute top-0 right-0 min-w-[46px] min-h-[46px] flex items-center justify-center rounded-2xl border transition-all duration-150 active:scale-90 shadow-sm ${
                isSpeaking
                  ? 'bg-amber-500 text-white animate-pulse border-amber-400'
                  : isDarkMode
                  ? 'bg-[#3B4D61] border-[#5E7A94] text-blue-300 hover:bg-[#2D3B4A]'
                  : 'bg-[#E8F0EC] border-[#B7E4C7] text-[#1E4B3D] hover:bg-[#D8E2DC]'
              }`}
              aria-label="Multilingual Audio Readout (Hindi/Odia/English)"
          ...
          {submitSuccess && (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs opacity-80">Directly recorded in District Planning Registry.</p>
              </div>
            </div>
          )}
          ...
          {photoPreview && (
            ...
            >
              ...
              <button
                ...
                className="absolute top-1.5 right-1.5 p-1 rounded-full bg-slate-900/80 text-white hover:bg-red-600 active:scale-95 transition-all"
              >
                <X className="w-4 h-4" />
              </button>
              <div
          ...
          {isRecording && (
            <div
              className={`mb-4 rounded-2xl border-2 p-3 text-center shadow-xl ${
                isDarkMode ? 'bg-[#3B4D61] border-blue-400' : 'bg-[#1E4B3D] border-[#52B788]'
              }`}
            >
          ...
          <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
            ...
            {/* The Central Unified Input Bar */}
            <div
              className={`w-full rounded-2xl border-2 transition-all duration-200 shadow-inner p-2 flex items-center gap-2 ${
                isRecording
                  ? 'border-red-500 ring-4 ring-red-500/20'
                  : isDarkMode
                  ? 'bg-[#3B4D61] border-[#5E7A94] focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-400/20'
                  : 'bg-[#F4F9F6] border-[#B7E4C7] hover:border-[#52B788] focus-within:border-[#1E4B3D] focus-within:ring-4 focus-within:ring-[#1E4B3D]/15'
              }`}
            >
                ...
                <button
                  ...
                  className={`relative min-w-[58px] min-h-[58px] rounded-xl flex items-center justify-center transition-all duration-150 active:scale-95 ${
                    isRecording
                      ...
                      : isDarkMode
                      ? 'bg-[#4A6278] text-blue-300 hover:bg-[#5E7A94] border border-[#5E7A94]'
                      : 'bg-white text-[#1E4B3D] hover:bg-[#E8F0EC] border border-[#B7E4C7]'
                  }`}
              ...
              <input
                ...
                className={`flex-1 bg-transparent text-base md:text-lg font-medium px-2 py-3 outline-none border-none ${
              ...
              <button
                ...
                className={`min-w-[58px] min-h-[58px] rounded-xl flex items-center justify-center transition-all duration-150 active:scale-95 border ${
                  isDarkMode
                    ? 'bg-[#4A6278] text-blue-300 hover:bg-[#5E7A94] border-[#5E7A94]'
                    : 'bg-white text-[#1E4B3D] hover:bg-[#E8F0EC] border-[#B7E4C7]'
                }`}
            ...
            <button
              type="submit"
              className={`w-full min-h-[62px] rounded-2xl text-white font-bold text-base md:text-lg shadow-lg flex items-center justify-center gap-3 transition-all duration-150 active:scale-95 hover:-translate-y-0.5 ${
                isDarkMode
          ...
          <div className="mt-6 pt-4 border-t border-dashed border-slate-300/40 text-center">
            <p className={`text-xs font-semibold uppercase tracking-wider mb-2.5 ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
              ⚡ One-Touch Voice Simulation:
            </p>
            <div className="flex flex-wrap justify-center gap-2">
              <button
                ...
                className={`text-xs font-medium px-3.5 py-1.5 rounded-full border transition-all active:scale-95 shadow-sm ${
                  isDarkMode
                    ? 'bg-[#3B4D61] border-[#5E7A94] text-white hover:bg-[#2D3B4A]'
      ...
      {/* -------------------------------------------------------------
          3. SLIDE-OUT DRAWER MENU (All 8 Core Operational Tabs)
          ------------------------------------------------------------- */}
      {isDrawerOpen && (
